...
class MotBBSequence(Dataset):
    ...
    def __init__(self, paths_file, loader=default_loader, seq_length=20, new_height=416, new_width=416, step=5,
        ...
        self.sequences = []  # contains train and valid sequences
        # todo place the stuff below into a function with a good name
        for video in videos_train.keys():  # todo look up whether .keys returns sorted list
            video_len = len(videos_train[video])

            start_index = 0
            while True:
                sequence = np.zeros([seq_length, 120, 5])  # 120 boxes with 5 parameter
                if start_index + seq_length < video_len:
                    sub_sequence = videos_train[video][start_index: start_index + seq_length]
        ...
        for video in videos_valid.keys():
            video_len = len(videos_valid[video])

            start_index = 0
            while True:
                sequence = np.zeros([seq_length, 120, 5])  # 120 boxes with 5 parameter
                if start_index + seq_length < video_len:
                    sub_sequence = videos_valid[video][start_index: start_index + seq_length]
                    for j in range(seq_length):
                        sequence[j, :sub_sequence[j].shape[0], :] = sub_sequence[j][:, 1:6]
